...
int main(int argc,char* argv[]){
    if(argc!=9){fprintf(stdout,"Usage: icmp-smash-flood --source-addr saddr --remote-addr raddr --source-port sport --n N"); exit(0);}
	else{	
		...
		set_icmp_header(icmp_hdr,ICMP_TYPE_ROUTER_ADVERTISEMENT,0,0,0);
		...
		fprintf(stdout,"Sent %d icmp ROUTER ADVERTISEMENT to remote %s\n",count,source_address);
