...
int main(int argc,char* argv[]){
    if(argc!=9){ fprintf(stdout,"Usage: icmp-flood --source-addr saddr --remote-addr raddr --source-port sport --n N"); exit(0); }
	else{	
		...
		fprintf(stdout,"Sent %d icmp echos to remote %s\n",count,source_address);
