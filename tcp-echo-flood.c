...
int main(int argc,char* argv[]){
	srand(time(NULL));
    if(argc!=7) fprintf(stdout,"Usage: tcp-echo-flood --remote-addr raddr --remote-port rport --n N");
	else{	
		...
		set_tcp_header(tcp_hdr,source_port,remote_port,0,0,TCP_DEFAULT_OFFSET,TCP_DEFAULT_RESERVED,0,0,0,0,0,0,1,0, \
